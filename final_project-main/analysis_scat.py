...
ff = np.fft.fft(m1_1[:,int(DATASET_ID)-1])

ff_conj = ff*ff.conj()
spectrum_ff = 2 * 0.000033 / m1_1.shape[0] * ff_conj
m1_fft = spectrum_ff[:int(ff.shape[0] / 2),:]
dset = str(int(DATASET_ID)-1)
np.save(HOME+'/hw4/ses'+SES_ID+'/ch'+dset+'_real.npy',m1_fft[:13905])
"""
